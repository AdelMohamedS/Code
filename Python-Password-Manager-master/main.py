...
from modules.exceptions import PasswordFileDoesNotExist, UserExits
...
def start(obj: DataManip):
    if os.path.isfile("db/masterpassword.json"):
        ...
        spinner = Halo(
            text=colored("Unlocking", "green"), color="green", spinner=obj.dots_
        )
        if sha256(master_password.encode("utf-8")).hexdigest() == stored_master_pass:
            ...
            menu = Manager(
                obj, "db/passwords.json", "db/masterpassword.json", master_password
            )
            ...
            try:
                ...
                print(
                    colored(
                        f"{obj.x_mark_} DB not found. Try adding a password {obj.x_mark_}",
                        "red",
                    )
                )
        else:
            print(
                colored(
                    f"{obj.x_mark_} Master password is incorrect {obj.x_mark_}", "red"
                )
            )
            return start(obj)
        ...
        print(
            colored(
                "To start, we'll have you create a master password. Be careful not to lose it as it is unrecoverable.",
                "green",
            )
        )
        ...
        if master_password == second_input:
            spinner = Halo(
                text=colored("initializing base...", "green"),
                color="green",
                spinner=obj.dots_,
            )
            ...
            print(
                colored(
                    f"{obj.checkmark_} Thank you! Restart the program and enter your master password to begin.",
                    "green",
                )
            )
        else:
            print(
                colored(
                    f"{obj.x_mark_} Passwords do not match. Please try again {obj.x_mark_}",
                    "red",
                )
            )
            return start(obj)
